...
class VoteSecureAPI {
    ...
    try {
      ...
      if (!firebase.apps.length) {
        firebase.initializeApp({
          apiKey: (typeof process !== 'undefined' && process.env && process.env.FIREBASE_API_KEY) || "your-api-key",
          authDomain: (typeof process !== 'undefined' && process.env && process.env.FIREBASE_AUTH_DOMAIN) || "your-project.firebaseapp.com",
          projectId: (typeof process !== 'undefined' && process.env && process.env.FIREBASE_PROJECT_ID) || "your-project-id",
          storageBucket: (typeof process !== 'undefined' && process.env && process.env.FIREBASE_STORAGE_BUCKET) || "your-project.appspot.com",
          messagingSenderId: (typeof process !== 'undefined' && process.env && process.env.FIREBASE_MESSAGING_SENDER_ID) || "123456789",
          appId: (typeof process !== 'undefined' && process.env && process.env.FIREBASE_APP_ID) || "your-app-id"
        });
      }

      // Check if real Firebase is available or if we are using placeholders
      ...
      auth.onAuthStateChanged(async (user) => {
        if (user) {
          try {
            ...
            console.error('Failed to get Firebase token, using local storage token if present', e);
          }
        } else {
          // Only clear if we were not in local auth mode
          if (!this.isMockMode) {
            this.token = null;
            this.user = null;
  ...
  async login(email, password) {
    // If not in mock mode, try Firebase login first
    if (!this.isMockMode && typeof firebase !== 'undefined' && firebase.apps.length) {
      try {
        ...
        console.warn('Firebase login failed, trying local login fallback...', error.message);
      }
    }

    // Local login fallback
    try {
      const response = await fetch(`${this.baseURL}/auth/login-local`, {
        method: 'POST',
      ...
      throw new APIError(error.message || 'Authentication failed', 401, 'LOGIN_ERROR');
    }
  }
  ...
  async register(userData) {
    ...
    // If not in mock mode, try to register on Firebase first
    if (!this.isMockMode && typeof firebase !== 'undefined' && firebase.apps.length) {
      try {
        ...
        console.warn('Firebase user creation failed, trying local register fallback...', error.message);
      }
    }
    ...
    try {
      ...
      // If we registered locally, login to establish session and get local JWT token
      if (!this.token) {
      ...
      if (error instanceof APIError) throw error;
      throw new APIError(error.message || 'Registration failed', 400, 'REGISTER_ERROR');
    }
  }
