...
// @desc    Login user with email and password (local mock authentication fallback)
// @access  Public
router.post('/login-local', async (req, res) => {
  try {
    ...
    const token = jwt.sign(
      { uid: user.firebaseUid, email: user.email, name: user.fullName },
      process.env.JWT_SECRET || 'votesecure-fallback-secret',
      { expiresIn: '24h' }
    );
