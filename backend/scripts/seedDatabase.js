...
async function seedDatabase() {
  try {
    ...
    // Connect to database
    await mongoose.connect(process.env.MONGODB_URI || process.env.MONGO_URI || 'mongodb://localhost:27017/votesecure');
    ...
    const elections = await Election.insertMany(sampleElections.map(election => ({
      ...election,
      status: 'draft', // Insert as draft first to avoid candidate-check validation
      workflow: {
    ...
    console.log(`✅ Created ${elections.length} draft elections`);
    ...
    // Set final statuses now that candidates are linked
    elections[0].status = 'published';
    elections[1].status = 'active';
    elections[2].status = 'published';
    
    await Promise.all(elections.map(election => election.save()));
    console.log('✅ Linked candidates to elections and updated statuses');
